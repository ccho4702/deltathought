...
ego4d-commit-timing-smoke:
	CUDA_VISIBLE_DEVICES=$(EVAL_GPU_ID) uv run --frozen deltaomni-train-ego4d-commit-timing \
		--config configs/ego4d_commit_timing_smoke.yaml

ego4d-commit-timing:
	CUDA_VISIBLE_DEVICES=$(EVAL_GPU_ID) uv run --frozen deltaomni-train-ego4d-commit-timing \
		--config configs/ego4d_commit_timing.yaml
