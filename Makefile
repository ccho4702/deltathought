...
evaluate-longvideobench-video-smoke:
	CUDA_VISIBLE_DEVICES=$(EVAL_GPU_ID) uv run --frozen \
		deltaomni-evaluate-longvideobench-video \
		--config configs/longvideobench_video_qa_smoke.yaml
...
ego4d-goalstep-caption-multineg-smoke:
	CUDA_VISIBLE_DEVICES=$(GPU_IDS) uv run --frozen torchrun --standalone \
		--nproc-per-node=$(NPROC_PER_NODE) -m deltaomni.ego4d_goalstep_caption_lora \
		--config configs/ego4d_goalstep_caption_multineg_smoke.yaml

ego4d-goalstep-caption-multineg:
	CUDA_VISIBLE_DEVICES=$(GPU_IDS) uv run --frozen torchrun --standalone \
		--nproc-per-node=$(NPROC_PER_NODE) -m deltaomni.ego4d_goalstep_caption_lora \
		--config configs/ego4d_goalstep_caption_multineg.yaml
